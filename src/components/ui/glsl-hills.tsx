...
class HillsPlane {
  ...
  constructor(planeSize: number, speed: number) {
    ...
    // The vertex shader rotates + displaces the plane on the GPU, so the CPU-side
    // bounding sphere (computed from the flat, undisplaced geometry) can't be
    // trusted for frustum culling.
...
export function GLSLHills({
  ...
  useEffect(() => {
    ...
    const plane = new HillsPlane(planeSize, speed);
    ...
    // requestAnimationFrame already pauses natively in backgrounded tabs,
    // so no extra Page Visibility bookkeeping is needed here.
    const loop = () => {
      plane.render(reduceMotion ? 0 : clock.getDelta());
      renderFrame();
